import { pgTable, serial, varchar, text, timestamp, integer, boolean, date, pgEnum, decimal } from 'drizzle-orm/pg-core';
...
export const contractStatusEnum = pgEnum('contract_status', ['CONTRACT', 'PERMANENT']);
...
export const contracts = pgTable('contracts', {
  ...
  statusContract: contractStatusEnum('status_contract').default('CONTRACT').notNull(),
  ...baseFields,
});
...
export const criteriaVersions = pgTable('criteria_versions', {
  id: serial('id').primaryKey(),
  criteriaId: integer('criteria_id').references(() => criteria.id, { onDelete: 'cascade' }).notNull(),
...
export const recordCategoryEnum = pgEnum('record_category', ['QUALITY', 'RESPONSIBILITY']);
export const metricTypeEnum = pgEnum('metric_type', ['PRODUCTION', 'ATTENDANCE']);

export const dailyRecords = pgTable('daily_records', {
  id: serial('id').primaryKey(),
  employeeId: integer('employee_id').references(() => employees.id, { onDelete: 'cascade' }).notNull(),
  supervisorId: integer('supervisor_id').references(() => employees.id).notNull(),
...
export const assessments = pgTable('assessments', {
  id: serial('id').primaryKey(),
  reportId: integer('report_id').references(() => reports.id, { onDelete: 'cascade' }).notNull(),
  criteriaVersionId: integer('criteria_version_id').references(() => criteriaVersions.id).notNull(),
...
export const criteriaVersionsRelations = relations(criteriaVersions, ({ one, many }) => ({
  criteria: one(criteria, { fields: [criteriaVersions.criteriaId], references: [criteria.id] }),
  assessments: many(assessments),
}));

export const dailyRecordsRelations = relations(dailyRecords, ({ one }) => ({
  employee: one(employees, { fields: [dailyRecords.employeeId], references: [employees.id] }),
  supervisor: one(employees, { fields: [dailyRecords.supervisorId], references: [employees.id] }),
}));

export const externalMetricsRelations = relations(externalMetrics, ({ one }) => ({
  employee: one(employees, { fields: [externalMetrics.employeeId], references: [employees.id] }),
}));
...
export const assessmentsRelations = relations(assessments, ({ one }) => ({
  report: one(reports, { fields: [assessments.reportId], references: [reports.id] }),
  criteriaVersion: one(criteriaVersions, { fields: [assessments.criteriaVersionId], references: [criteriaVersions.id] }),
}));
